...
function compute_nllh(x, oprob::ODEProblem, solver, measurements::DataFrame; abstol = 1e-9,
    ...
    _u0 = convert.(eltype(x), ComponentArray(oprob.u0))
    _p[1:4] .= x[1:4]
    _u0[1] = nnout[1]
    _oprob = remake(oprob, p = _p, u0 = _u0)
...
# Write problem yaml
mapping_table = DataFrame(petabEntityId = ["input1", "input2", "prey"],
