...
function compute_nllh(x, oprob::ODEProblem, solver, measurements::DataFrame, pnet1;
    ...
    for i in eachindex(mprey)
        model_output = nn_model([prey[i], predator[i]], pnet1, st)[1][1]
...
# Objective function
x = ComponentArray((α = 1.3, δ = 1.8, β = 0.9, γ = 0.8))
...
prey_vals = [nn_model(sol[:, i], pnet1, st)[1][1] for i in eachindex(sol.t)]
