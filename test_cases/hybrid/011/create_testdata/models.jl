...
function _lv!(du, u, p, t, nn)
    prey, predator = u
    @unpack δ, β, p_net1, p_net2 = p

    st1, nn1 = nn[:net1]
    du_nn1 = nn1([prey, predator], p_net1, st1)[1]
    st2, nn2 = nn[:net2]
    du_nn2 = nn2([prey, predator], p_net2, st2)[1]

    du[1] = du_nn2[1] - β * prey * predator # prey
    du[2] = du_nn1[1] - δ*predator # predator
    return nothing
end
lv! = let nn = nndict
...
# UDE
p_mechanistic = (δ = 1.8, β = 0.9)
p_ode = ComponentArray(merge(p_mechanistic, (p_net1=pnn1, p_net2 = pnn2)))
