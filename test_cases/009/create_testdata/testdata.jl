...
function compute_nllh(x, oprob::ODEProblem, solver, measurements::DataFrame; abstol = 1e-9,
    ...
    nnout1 = nn_model1([1.0, 1.0], x.p_net1, st1)[1]
...
_p = (α = 1.3, δ = 1.8, β = nn_model2([2.0, 2.0], x.p_net2, st2)[1][1], γ = nn_model1([1.0, 1.0], x.p_net1, st1)[1][1])
_oprob = remake(oprob, p = _p)
...
mapping_table = DataFrame(
    Dict("petab.MODEL_ENTITY_ID" => ["net1.input1", "net1.input2", "net1.output1",
         ...
         "petab.PETAB_ENTITY_ID" => ["input1", "input2", "gamma",
                                     "input3", "input4", "beta"]))
