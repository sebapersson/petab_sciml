...
The `petab_sciml` library provides a YAML neural network file format for model exchange. The YAML format follows PyTorch conventions for layer names and arguments. The schema is provided as [YAML-formatted JSON schema](@__STATIC__/net_schema.yaml), which enables easy validation with various third-party tools.
